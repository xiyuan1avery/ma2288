...
```math
\hat{h}_{t+1} = h_t + f_{\psi}\left(\mathrm{LN}\left[h_t; E\left(x_{t+1}\right)\right]\right)
```
...
```math
L_{\mathrm{roll}} = \frac{1}{8}\sum_{k=1}^{8}\mathrm{SmoothL1}\left(\hat{h}_{t+k}, h_{t+k}\right)
```
